...
import fpt.capstone.etbs.service.*;
...
@Service public class PublishServiceImpl implements PublishService
{
    ...
    public Publish createPublish(UUID authorId, PublishRequest request)
    {
        ...
        Publish publish = Publish.builder().content(request.getContent()).name(request.getName()).description(request.getDescription()).author(author).status(PublishStatus.PENDING).build();
    ...
    public void checkDuplicate()
    {
        ...
        List<PublishResponse> responses = publishes.stream().map(PublishResponse::setResponse).sorted(Comparator.comparing(PublishResponse::getRequestDate).reversed()).collect(Collectors.toList());
    ...
    public void checkDuplicate(UUID authorId, Publish publish)
    {
        ...
        publishes = publishes.stream().filter(p -> p.getAuthor().getId().equals(authorId)).collect(Collectors.toList());
        responses = publishes.stream().map(PublishResponse::setResponse).sorted(Comparator.comparing(PublishResponse::getRequestDate).reversed()).collect(Collectors.toList());
    ...
    private boolean checkPublishPolicy(UUID authorId)
    {
        ...
        long count = publishRepository.countByAuthor_IdAndStatusInAndCreatedDateBetween(authorId, statuses, startDay, endDay);
