...
public class UserController {
  ...
  @PutMapping("/user")
  public ResponseEntity<ApiResponse> updateUser(@Valid @RequestBody AccountUpdateRequest request,
      Authentication auth) {
    UserPrincipal userPrincipal = (UserPrincipal) auth.getPrincipal();
    Account account = accountService.getAccount(userPrincipal.getId());
    ...
    try {
      ...
      if (account.getProvider().equals(AuthProvider.google)) {
        return ResponseEntity
            .ok(new ApiResponse<>(true, "Your account wasn't create with local", null));
      }
